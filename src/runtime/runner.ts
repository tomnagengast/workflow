...
export class WorkflowRunner {
  ...
  async run(workflow: WorkflowRow, argsValue: unknown): Promise<unknown> {
    ...
    const bag = buildSandboxBag({
      ...
      log: (message) => console.error(`[workflow:${workflow.name}] ${message}`),
  ...
  async _dispatch(
    ...
    if (this.runtime.budget != null && this.spent >= this.runtime.budget) {
      throw new BudgetError(`token budget exhausted (${this.spent}/${this.runtime.budget})`);
    }
    if (opts.agentType) console.error(`[workflow:${workflow.name}] note: agentType '${opts.agentType}' ignored`);
    if (opts.isolation === "worktree") console.error(`[workflow:${workflow.name}] note: isolation 'worktree' is advisory in this runner`);
    if (!BACKENDS[backendName]) {
      console.error(`[workflow:${workflow.name}] ${kind} FAILED (-> null): backend '${backendName}' unavailable`);
      return null;
    }

    this.journal({ type: "started", key, agentId: label, backend: backendName, kind });
    ...
    try {
      ...
      this.journal({ type: "result", key, agentId: label, backend: backendName, kind, result: value });
      ...
      this.journal({ type: "result", key, agentId: label, backend: backendName, kind, result: null, error: String(err.message || err) });
      return null;
    } finally {
      this.sem.release();
    }
  ...
  async workflow(nameOrSpec: unknown, nestedArgs?: unknown): Promise<unknown> {
    ...
    if (typeof nameOrSpec === "object" && nameOrSpec !== null) {
      ...
      if (spec.scriptPath) {
        ...
        return this.runNested(nested, spec.args);
      }
      name = spec.name;
      argsValue = spec.args;
    }
    if (typeof name !== "string") throw new Error("workflow() expects a workflow name or {scriptPath}");
    return this.runNested(requireWorkflow(this.workflows, name), argsValue);
  }
  ...
  async runNested(workflow: WorkflowRow, argsValue: unknown): Promise<unknown> {
    ...
    const bag = buildSandboxBag({
      args: argsValue,
      budget: this.makeBudget(),
      phase: (t) => { this.phaseTitle = String(t); console.error(`\n[workflow:${workflow.name}] phase: ${this.phaseTitle}`); },
      log: (m) => console.error(`[workflow:${workflow.name}] ${m}`),
      agent: (prompt, opts = {}) => this.agent(workflow, prompt, opts),
      gate: (prompt, opts = {}) => this.gate(workflow, prompt, opts),
      parallel: (thunks) => this.parallel(thunks),
      pipeline: (items, ...stages) => this.pipeline(items, stages),
